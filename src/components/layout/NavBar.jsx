...
const Navbar = () => {
    ...
    const themeNight = "bg-black border-gray-700"
    const themeLight = "bg-white border-slate-200"

return (
    <nav className={`${theme === 'light' ? themeLight : themeNight} gap-6 border-b px-8 py-4 flex items-center justify-between sticky top-0 z-10`}>
        <div className=" md:gap-16 flex items-center gap-8">
        <h1 className=" xl:text-xl md:text-[20px] font-black text-blue-600 tracking-tighter uppercase ">Clinica Estética</h1>


        {theme === 'light' ? 
        <div className={`flex  ${themeLight} h-14 p-1 rounded-xl`}>
            {filtros.map((f) => (
            <button
                
                key={f.id}
                onClick={() => setActiveFilter(f.id)}
                className={`flex md:h-14 text-[12px]  xl:text-[14px]  justify-center items-center gap-2 px-3 py-2 rounded-lg text-sm font-bold transition-all ${
                activeFilter === f.id
        ...
        :
        <div className={`flex ${themeNight} h-14 p-1 rounded-xl`}>
            {filtros.map((f) => (
            <button
        ...
        <div>
            <Field >
                <Input 
                className={`${theme === 'light' ? " sm:w-30 xl:w-55  focus-visible:ring-blue-400": " xl:w-55  md:w-38 focus-visible:ring-white"}`} id="input-button-group" 
